...
import static org.mule.runtime.core.api.context.notification.EnrichedNotificationInfo.create;
  ...
  protected void fireNotification(Exception ex, Event event) {
    if (enableNotifications) {
      if (ex.getCause() != null && getCause(ex) instanceof SecurityException) {
        ...
        fireNotification(new ExceptionNotification(ex, create(event, ex, null), flowConstruct));
      }
    }
  }
